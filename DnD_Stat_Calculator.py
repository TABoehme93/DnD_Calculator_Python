...
class DnD_Calculator:
    def __init__(self):
        ...
        self.score = tkinter.Label(self.scoreFrame, text="SCORE")#Thomas Boehme
#started: 2017/3/14
#last edited: 2019/8/11
#finished: --
#Quick stat calculator for D&D, starting with 4e stat calculations
#4e due to how simple the system is for calculations
#5e will be next, followed by 3.5 then 2e.
#Next goals:
    #add level input, this includes modifiers + half level
    #calculate skills
    #utilize a database to add racial modifiers
...
class DnD_Calculator:
    def __init__(self):
        ...
        self.lvlError = tkinter.Label(self.levelRow, text="")
        self.lvlValid = False   #used to see if the current level input is valid
        self.lvlNum = 0         #just a dummy number of input
    ...
    def display_Stats(self):
        ...
        self.strMod.config(text=self.get_Modifier(self.strEntry.get())+"")
        self.conMod.config(text=self.get_Modifier(self.conEntry.get())+"")
        self.dexMod.config(text=self.get_Modifier(self.dexEntry.get())+"")
        self.intMod.config(text=self.get_Modifier(self.intEntry.get())+"")
        self.wisMod.config(text=self.get_Modifier(self.wisEntry.get())+"")
        self.chaMod.config(text=self.get_Modifier(self.chaEntry.get())+"")

        self.strModHalf.config(text=self.get_ModHalf(self.strEntry.get())+"")
        self.conModHalf.config(text=self.get_ModHalf(self.conEntry.get())+"")
        self.dexModHalf.config(text=self.get_ModHalf(self.dexEntry.get())+"")
        self.intModHalf.config(text=self.get_ModHalf(self.intEntry.get())+"")
        self.wisModHalf.config(text=self.get_ModHalf(self.wisEntry.get())+"")
        self.chaModHalf.config(text=self.get_ModHalf(self.chaEntry.get())+"")
    ...
    def level_Check(self):  #checks if the level is valid
        try:
            self.lvlNum = int(self.lvlEntry.get()+"")
            self.lvlError.config(text="")
            self.lvlValid = True
        except ValueError:
            self.lvlError.config(text="not a valid level")
            self.lvlValid = False
    #end level_Check method

    def get_Modifier(self, givenStat): 
    ...
    def get_ModHalf(self, givenStat):
        if (self.lvlValid == False):
            return "level not valid"
        modifier = self.get_Modifier(givenStat)
        try:
            modHalf = int(modifier) + (self.lvlNum//2)
            if (modHalf > 0):
                return ("+" + str(modHalf))
            else:
                return str(modHalf)
